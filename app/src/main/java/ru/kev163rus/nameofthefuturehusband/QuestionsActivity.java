package ru.kev163rus.nameofthefuturehusband;
...
public class QuestionsActivity extends Activity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setQuestionAndAnswers(AccessoryClass.indexOfQuestion);

        arrayOfDialogs = new int[10];
        fillArrayOfDialogs(arrayOfDialogs);

        AccessoryClass.isDebuging = false;    // set "false" in release.

        // Реклама.
        Appodeal.disableLocationPermissionCheck();
        Appodeal.setTesting(AccessoryClass.isDebuging);
    ...
    private void setQuestionAndAnswers(int indexOfQuestion){
        ...
        String textToTextView = String.valueOf(AccessoryClass.indexOfQuestion) + " " + getString(R.string.textViewQuestionCountOfQuestions) + " " + String.valueOf(AccessoryClass.countOfQuestions);
        textViewQuestionCountOfQuestions.setText(textToTextView);
        textViewQuestionText.setText(getQuestionText(AccessoryClass.arrayOfNumQuestions[indexOfQuestion - 1]));
        textViewQuestionAnswer1.setText(getAnswer1Text(AccessoryClass.arrayOfNumQuestions[indexOfQuestion - 1]));
        textViewQuestionAnswer2.setText(getAnswer2Text(AccessoryClass.arrayOfNumQuestions[indexOfQuestion - 1]));
        textViewQuestionAnswer3.setText(getAnswer3Text(AccessoryClass.arrayOfNumQuestions[indexOfQuestion - 1]));
    ...
    public void setNewQuestion(boolean isIncrement, int userChoise){
        ...
        if (arrayOfDialogs != null && AccessoryClass.indexOfQuestion % 4 == 0) setDialog(arrayOfDialogs[(AccessoryClass.indexOfQuestion / 4) - 1]);

        AccessoryClass.setUserResult(AccessoryClass.indexOfQuestion - 1, userChoise);

        if (isIncrement & AccessoryClass.indexOfQuestion < AccessoryClass.countOfQuestions){
            AccessoryClass.indexOfQuestion++;
        }else if (!isIncrement & AccessoryClass.indexOfQuestion > 1) {
            AccessoryClass.indexOfQuestion--;
        }else if (isIncrement & AccessoryClass.indexOfQuestion == AccessoryClass.countOfQuestions){
            finishTest();
        }
